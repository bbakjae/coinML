...
class Prediction:
    ...
    interval: str
    filename: str
    window_size: int  # 예측에 활용할 과거 데이터 개수
    batch_size: int  # 분할해서 학습시킬 정보 보통 32 or 64

    def __init__(self, market, interval, filename, window_size=5, batch_size=32):
        self.market = market
    ...
    def windowed_dataset(
        ...
        ds_x = ds_x.window(self.window_size, shift=1, stride=1, drop_remainder=True)
        ds_x = ds_x.flat_map(lambda x: x.batch(self.window_size))
        # y값 추가
        ds_y = tf.data.Dataset.from_tensor_slices(y[self.window_size :])
    ...
    def show_history(self, history, test_data, y_test):
        print("%.7f" % (float(min(history.history["val_loss"]))))
        self.model.load_weights(self.filename)
        pred = self.model.predict(test_data)
        actual = np.asarray(y_test)[self.window_size :]
    ...
    def get_live_dataframe(self) -> DataFrame:
        ...
        scaled_df = self.scale_dataframe(df=df)
        live_x, live_y = scaled_df.drop(columns="close"), scaled_df["close"]
        live_df = self.windowed_dataset(live_x, live_y, False)
        return live_df

    def get_predict(self, df: DataFrame) -> float:
        self.model.load_weights(self.filename)
    ...
    def learning_data(self):
        ...
        x_train, x_test, y_train, y_test = self.df_train_test_split(df)
        train_data = self.windowed_dataset(x_train, y_train, True)
        test_data = self.windowed_dataset(x_test, y_test, False)
